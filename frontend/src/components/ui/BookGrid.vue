    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn style="margin-left: 5px;" @click="requestbookAuthorityDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('reader')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>도서 권한 요청
                </v-btn>
                <v-dialog v-model="requestbookAuthorityDialog" width="500">
                    <RequestbookAuthority
                        @closeDialog="requestbookAuthorityDialog = false"
                        @requestbookAuthority="requestbookAuthority"
                    ></RequestbookAuthority>
                </v-dialog>
            </div>
                ...
                <v-card>
                    ...
                    <v-card-text>
                        <div>
                            ...
                            <File offline label="Contents" v-model="selectedRow.contents" :editMode="true"/>
...
export default {
    ...
    data: () => ({
        path: 'books',
        requestbookAuthorityDialog: false,
    }),
    watch: {
    },
    methods:{
        async requestbookAuthority(params){
            try{
                var path = "requestbookAuthority".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, params)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','requestbookAuthority 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
                }
                this.requestbookAuthorityDialog = false
            }catch(e){
                console.log(e)
            }
        },
